...
#[cfg(windows)]
static BACKEND_BYTES: &[u8] = include_bytes!("../../binaries/backend-x86_64-pc-windows-msvc.exe");
...
fn wait_for_ready(host: &str, port: u16, timeout_secs: u64) -> bool {
...
#[cfg(windows)]
pub fn spawn_and_wait(app: &tauri::AppHandle) -> Child {
    ...
    {
        let mut file = fs::File::create(&backend_path).expect("failed to create backend file");
    ...
    if !wait_for_ready(BACKEND_HOST, BACKEND_PORT, 15) {
